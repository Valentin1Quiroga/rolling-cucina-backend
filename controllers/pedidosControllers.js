...
const getPedido = async (req,res) =>{
    try {
        ...
        res.status(error.code || 500).json({message:error.message || "Ha ocurrido un problema inesperado. Por favor intente de nuevo mas tarde." })
    }
};
...
const addPedido = async (req,res) =>{
    try {
        ...
        res.status(201).json({message:"El pedido se realizó correctamente y está pendiente de realizarse. Pronto comenzaran a prepararlo. Muchas gracias.", pedido})
    } catch (error) {
        res.status(error.code || 500).json({message:error.message || "Ha ocurrido un problema inesperado. Por favor intente de nuevo mas tarde." })
    }
};
...
const deletePedido = async (req, res) => {
    try {
      ...
      if (!pedidoRemoved) throw new CustomError("El pedido no se encuentra en nuestra base de datos.", 404);
